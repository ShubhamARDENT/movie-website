        ...
        >
            ...
            <div className="flex justify-between gap-3 flex-wrap">
                {pokemon.types.map((type, index) => {
                    ...
                    return <span key={index} className={`${typeColor} text-sm color px-5 py-2 font-bold rounded-md`}>{type.type.name}</span>
                })}
            </div>
            ...
            <div className='flex justify-between my-5 gap-2'>
                ...
                <div className='flex flex-col items-center'>
                    ...
                    <span className='bg-gray-200 
                   font-semibold px-[43px] py-[5px] rounded-3xl'>{pokemon.weight}Kg</span>
                </div>
            </div>
            <div className='flex justify-between gap-2'>
                <div className='flex flex-col items-center'>
                ...
                <div className='flex flex-col items-center'>
                    ...
                    <span className='bg-gray-200 
                   font-semibold px-[55px] py-[5px] rounded-3xl'>{pokemon.base_experience}</span>
                </div>
            </div>
            <div className='mt-5 text-center'>
                ...
                <ul className='flex gap-2 mt-5 justify-between' >
                    {pokemon.stats.map((stat, index) => {
                        ...
                        return (
                            <li key={index} className='bg-gray-200 px-1 py-1 rounded-t-full rounded-b-full'>
                                <span className='font-bold'>
                                    <span className={`${statColor} rounded-full px-[7px] py-[6px] font-semibold text-[10px] text-white flex flex-col`}>
                                        {statName}
                                    </span>
                                    {stat.base_stat}
                                </span>
